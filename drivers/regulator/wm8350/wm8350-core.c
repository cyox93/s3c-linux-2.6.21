/*
 * wm8350_core.c  --  Power Management Driver for Wolfson WM8350 PMIC
 *
...
#include <linux/mfd/wm8350/cache.h>
				  ...
				  int bytes, char *dest)
{
	...
	if (bytes > 2) {
		...
		while (bytes) {
			ret = wm8350_read_spi_device(wm8350, reg, 2, dest);
			if (ret != 2)
				return ret;
				   ...
				   int bytes, char *src)
{
	...
	if (bytes > 2) {
		while (bytes) {
			ret = wm8350_write_spi_device(wm8350, reg, 2, src);
			if (ret != 2)
				return ret;
...
static int wm8350_write(struct wm8350 *wm8350, u8 reg, int num_regs, u16 * src)
{
	...
	for (i = reg; i < end; i++) {
		if (!wm8350_reg_io_map[i].writable) {
			printk(KERN_ERR
			       "wm8350: attempted write to read only reg R%d\n",
			       i);
			return -EINVAL;
		}
		...
		if (is_reg_locked(wm8350, i)) {
			printk(KERN_ERR
			       "wm8350: attempted write to locked reg R%d\n",
			       i);
			return -EINVAL;
		}
	}
...
static int gpio_set_debounce(struct wm8350 *wm8350, int gpio, int db)
{
	if (db == WM8350_GPIO_DEBOUNCE_ON)
		return wm8350_set_bits(wm8350, WM8350_GPIO_DEBOUNCE, 1 << gpio);
	else
		return wm8350_clear_bits(wm8350,
...
int wm8350_device_init(struct wm8350 *wm8350)
{
	...
	printk(KERN_INFO "wm8350: core version %s\n", WM8350_CORE_VERSION);

	/* enable WM8350 register caching for faster IO on slow busses */
	...
	if (id1 == 0x0)
		...
		case WM8350_REV_E:
			printk(KERN_INFO "wm8350: found Rev E device\n");
			wm8350->rev = WM8350_REV_E;
			break;
		case WM8350_REV_F:
			printk(KERN_INFO "wm8350: found Rev F device\n");
	...
	printk(KERN_INFO "wm8350: config mode %1x:%2.2x\n",
	       (id2 & WM8350_CONF_STS_MASK) >> 10, (id2 & WM8350_CUST_ID_MASK));
	return 0;
