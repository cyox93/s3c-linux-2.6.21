...
* FILE NAME                                  VERSION                
*                                                                       
* VIM_JPEG_Driver.c			   0.1                    
*                                                                       
* DESCRIPTION                                                           
*                                                                       
*     VC0578 's JPEG moudle  driver                                 
*
*	Version		Author			Date		Description		
*  ---------------------------------------------------------------						 
*   0.1			angela		2005-11-9	The first version. 
*  ---------------------------------------------------------------
*                                                                       
*************************************************************************/
/****************************************************************************
...
static UINT16 VIM_JPEG_CalCapDelay(UINT8 fmt, TSize JpegSize)
{
	UINT8	blkheight, comp,cx=JpegSize.cx;
	UINT32	dlycnt;
	...
	dlycnt=(JpegSize.cx*JpegSize.cy);// add x clk
	JpegSize.cy%=blkheight;
	dlycnt+=(JpegSize.cx+cx)*JpegSize.cy;// add y clk
	dlycnt-=7;
	dlycnt>>=3;
...
{
	UINT8 bMode,qf=VIM_JPEG_GetQFactor();
	
	VIM_JPEG_SetBrcQF(qf);
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT3);
#if(VIM_JPEG_DEBUG)&&(VIM_528RDK_DEBUG)
		VIM_USER_PrintDec("\n VIM_JPEG_ResetState qf=",qf);
#endif
	/* 2006-1-5 	angela added it for reset linebuf*/
	bMode=VIM_HIF_GetReg8(V5_REG_LBUF_LBUF_CNT_RESET);
	bMode|=1;
	VIM_HIF_SetReg8(V5_REG_LBUF_LBUF_CNT_RESET,bMode);	// reset line buf
}
...
{
UINT8 bValue=0,bBypass=0;
UINT8 bLineMode2=0,bLineMode=0;
	...
	bBypass&=(~BIT4);	
	//line buffer decode bypass disable 
	bLineMode2=VIM_HIF_GetReg8(V5_REG_LBUF_LBUF_CNT_RESET);
	bLineMode2&=(~(BIT4|BIT3));	
	switch(bMode)
		{
		...
		case VIM_JPEG_MODE_CAPTURE_MJPEG:
			...
			bLineMode|=01;		//set line buf capture mode
			break;
		case VIM_JPEG_MODE_CAPTURE_AVI:
			bValue|=00;			//set jpeg capture  mode
			bLineMode|=01;		//set line buf capture mode
			break;	
		...
		case VIM_JPEG_MODE_DISPLAY_YUV:
			...
			bLineMode2|=BIT4; 	//line buf decode bypass enable
			break;
...
{
	UINT8 byValue;
	byValue = VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_MODE);
	byValue&=(~BIT3);
	byValue|=(bEnable<<3);
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_MODE, byValue);
}
/********************************************************************************
* Description:
*	Enable/Disalbe custom quantization table.
...
* This is for each bit of V5_REG_JPEG_JPEG_CTRL controls one start operation.
**********************************************************************************/
void VIM_JPEG_StartEncode(void)
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT0);
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT1);
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT2);
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT4);
}
/********************************************************************************
* Description:
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, BIT5);
}
/********************************************************************************
* Description:
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_JPEG_CTRL, (UINT8)(byHTIndex << 6));
}
...
{
	return VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_STATUS);
}

BOOL VIM_JPEG_DecodeIsDone(void)
{
	UINT8 byValue = VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_STATUS);
	return(byValue & BIT1);
}
...
{
	return VIM_HIF_GetReg8(V5_REG_JPEG_QF);
}
/********************************************************************************
* Description:
...
{
	dwCount>>=2;
	VIM_HIF_SetReg8(V5_REG_JPEG_TARGET_WC_3, (UINT8)dwCount);
	VIM_HIF_SetReg8(V5_REG_JPEG_TARGET_WC_2, (UINT8)(dwCount >> 8));
	VIM_HIF_SetReg8(V5_REG_JPEG_TARGET_WC_1, (UINT8)(dwCount >> 16));
	VIM_HIF_SetReg8(V5_REG_JPEG_TARGET_WC_0, (UINT8)(dwCount >> 24));
}
UINT32 VIM_JPEG_GetTargetWordCount(void)
{
  UINT32 value32;
	value32=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_TARGET_WC_3);
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_TARGET_WC_2)<<8;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_TARGET_WC_1)<<16;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_TARGET_WC_0)<<24;
	value32<<=2;	
  return value32;	
  }
/********************************************************************************
...
{
	dwCount>>=2;
	VIM_HIF_SetReg8(V5_REG_JPEG_VIDEO_WC_3, (UINT8)dwCount);
	VIM_HIF_SetReg8(V5_REG_JPEG_VIDEO_WC_2, (UINT8)(dwCount >> 8));
	VIM_HIF_SetReg8(V5_REG_JPEG_VIDEO_WC_1, (UINT8)(dwCount >> 16));
	VIM_HIF_SetReg8(V5_REG_JPEG_VIDEO_WC_0, (UINT8)(dwCount >> 24));
}

UINT32 VIM_JPEG_GetVideoWordCount(void)
{
  UINT32 value32;
  	value32=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_VIDEO_WC_3);
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_VIDEO_WC_2)<<8;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_VIDEO_WC_1)<<16;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_VIDEO_WC_0)<<24;
	value32<<=2;	
  return value32;	
}
UINT32 VIM_JPEG_GetJpegWordCount(void)
{
  UINT32 value32;
  	value32=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_WC_3);
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_WC_2)<<8;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_WC_1)<<16;
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_WC_0)<<24;
	value32<<=2;	
  return value32;	
}

UINT32 VIM_JPEG_GetJpegBlockCount(void)
{
  UINT32 value32=0;
  	value32=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_BLOCKCOUNT_L);
	value32|=(UINT32)VIM_HIF_GetReg8(V5_REG_JPEG_BLOCKCOUNT_H)<<8;
 	return value32;	
}
/********************************************************************************
* Description:
...
void VIM_JPEG_SetImgSize(VIM_JPEG_YUVMODE bYUVmode,TSize ImgeSize)
{
	...
	//set jpeg image size	
	VIM_HIF_SetReg8(V5_REG_JPEG_WIDTH_L, (UINT8)ImgeSize.cx);
	VIM_HIF_SetReg8(V5_REG_JPEG_WIDTH_H, (UINT8)(ImgeSize.cx >> 8));
	VIM_HIF_SetReg8(V5_REG_JPEG_HEIGHT_L, (UINT8)ImgeSize.cy);
	VIM_HIF_SetReg8(V5_REG_JPEG_HEIGHT_H, (UINT8)(ImgeSize.cy >> 8));
	//Set line buf image width
	VIM_HIF_SetReg8(V5_REG_LBUF_WIDTH_L,(UINT8)LineBufSize.cx);
	VIM_HIF_SetReg8(V5_REG_LBUF_WIDTH_H,(UINT8)(LineBufSize.cx>>8));
...
UINT16 VIM_JPEG_GetFrameCount(void)
{
	...
	byByteLow = VIM_HIF_GetReg8(V5_REG_JPEG_FRAME_CNT_0);
	byByteHigh = VIM_HIF_GetReg8(V5_REG_JPEG_FRAME_CNT_1);

	return ((UINT16)byByteHigh << 8) | (UINT16)byByteLow;
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_QUAN_T, byData);
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_HUFF_T, byData);
}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_NUM_COMP_QUANT, byNum);
}
...
void VIM_JPEG_SetComponentParm(UINT8 byIndex, UINT16 wValue)
{
	...
	{
	case 0:
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP0_P1, (UINT8)wValue);
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP0_P0, (UINT8)(wValue >> 8));
		break;
	case 1:
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP1_P1, (UINT8)wValue);
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP1_P0, (UINT8)(wValue >> 8));
		break;
	case 2:
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP2_P1, (UINT8)wValue);
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP2_P0, (UINT8)(wValue >> 8));
		break;
	case 3:
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP3_P1, (UINT8)wValue);
		VIM_HIF_SetReg8(V5_REG_JPEG_COMP3_P0, (UINT8)(wValue >> 8));
		break;
	default:
		break;
	}
...
{
	VIM_HIF_SetReg8(V5_REG_JPEG_NUM_BLK_L, (UINT8)wNum);
	VIM_HIF_SetReg8(V5_REG_JPEG_NUM_BLK_H, (UINT8)(wNum >> 8));
}
...
{
	UINT8 bValue,bValue1;

	bValue=VIM_HIF_GetReg8(V5_REG_JPEG_DEC_BYPASS);
	bValue1=VIM_HIF_GetReg8(V5_REG_LBUF_YUV_MODE);
	bPixelrate&=0x0f;
	//modify by amanda deng 2005-12-26 14:11
	bValue&=0xf0;
	bValue|=bPixelrate;
	VIM_HIF_SetReg8(V5_REG_JPEG_DEC_BYPASS,bValue);
	bValue1&=0x0f;
	bValue1|=(bPixelrate<<4);
	VIM_HIF_SetReg8(V5_REG_LBUF_YUV_MODE,bValue1);
}
...
{
	// line buf hblank
	VIM_HIF_SetReg8(V5_REG_LBUF_HBLANK_L,(UINT8)bHblank);
	VIM_HIF_SetReg8(V5_REG_LBUF_HBLANK_H,(UINT8)(bHblank>>8));
	bHblank++;
	// jpeg h blank
	VIM_HIF_SetReg8(V5_REG_JPEG_HBLANK_L,(UINT8)bHblank);
	VIM_HIF_SetReg8(V5_REG_JPEG_HBLANK_H,(UINT8)(bHblank>>8));
...
void VIM_JPEG_SetTimerInterval(UINT32 dwMs,UINT32 dwMclk)
{
	UINT16 Num1Ms;
		...
		dwMs*=Num1Ms;
		VIM_HIF_SetReg8(V5_REG_JPEG_TIMER_VALUE_0,(UINT8)(dwMs>>24));
		VIM_HIF_SetReg8(V5_REG_JPEG_TIMER_VALUE_1,(UINT8)(dwMs>>16));
		VIM_HIF_SetReg8(V5_REG_JPEG_TIMER_VALUE_2,(UINT8)(dwMs>>8));
		VIM_HIF_SetReg8(V5_REG_JPEG_TIMER_VALUE_3,(UINT8)dwMs);
}
/********************************************************************************
* Description:
...
{
	return ((VIM_HIF_GetReg8(V5_REG_LBUF_LBUF_STATUS)>>2)&0x1f);
}
/********************************************************************************
* Description:
...
UINT8 VIM_JPEG_CheckDecodeOK(void)
{
	...
	return ((VIM_HIF_GetReg8(V5_REG_JPEG_JPEG_STATUS)>>1)&1);
}

//add by amanda deng 2005-12-31 14:10
/********************************************************************************
	In Work Mode:
...
********************************************************************************/
static UINT8 VIM_JPEG_GetPixRate()
{
	UINT16 dst_cx, dst_cy, src_cx, src_cy;
	UINT32 lcddelay;
	UINT32 rate;
	...
	if(((VIM_HIF_GetReg8(V5_REG_LBUF_LBUF_MODE)&0x7)==0x2)&&(Amode!=VIM_DISP_LINEBUF))
		return  1;
	VIM_IPP_GetWindowSize(&src_cx, &src_cy);
	VIM_IPP_GetDispalyWindowSize(&dst_cx,&dst_cy );
	...
	lcddelay = VIM_HIF_GetReg8(V5_REG_LCDC_DEINTV) + 2;
	//if (gVc0528_Info.LcdStatus.RotationMode)
	if((VIM_HIF_GetReg8(V5_REG_LCDC_RM)&0x7)%2)
		lcddelay=lcddelay*2;
...
********************************************************************************/
static UINT16 VIM_JPEG_GetHbanlk(UINT8 pixelrate)
{
	UINT16 dst_cx, dst_cy, src_cx, src_cy;
	UINT32 lcddelay;
	UINT32 hblank;
	...
	lcddelay = VIM_HIF_GetReg8(V5_REG_LCDC_DEDLY)+ VIM_HIF_GetReg8(V5_REG_LCDC_LINEINTV)+ 4;
	hblank = DIV_ROUND( lcddelay * dst_cy, src_cy);
	hblank = DIV_ROUND( hblank, pixelrate);
	if(hblank > HABLAN_MAX)
		hblank = HABLAN_MAX;

	hblank+=(VIM_HIF_GetReg8(V5_REG_LBUF_YUV_MODE)&0x3);
...
{
UINT8 bValue;
UINT16 wLineBuvDelay,wComp[3];

	// set line buf yuv mode
	bValue=VIM_HIF_GetReg8(V5_REG_LBUF_YUV_MODE);
	bValue&=0xf8;
	if(bYUVmode>=JPEG_UNKNOWN)
		return VIM_ERROR_YUV_UNKNOW;
	bValue|=bYUVmode;
	VIM_HIF_SetReg8(V5_REG_LBUF_YUV_MODE,bValue);
	
	// set jpeg and line buf image width
	VIM_JPEG_SetImgSize(bYUVmode,ImageSize);
	// set video word counter
	VIM_JPEG_GetJpegSize(bYUVmode,ImageSize,&ImageSize);//angela 2006-3-5
	...
	if((VIM_HIF_GetReg8(V5_REG_LBUF_LBUF_MODE)&0x7)==0x1)//capture mode
	{
		if(JPEG_420 == bYUVmode)
			bValue = (UINT8)( ImageSize.cy % 16 );
		else
			bValue = (UINT8)( ImageSize.cy % 8 );
		if (bValue)
		{
		wLineBuvDelay=VIM_JPEG_CalCapDelay(bYUVmode,ImageSize);
		VIM_HIF_SetReg8(V5_REG_LBUF_CAP_DLY_L,(UINT8)wLineBuvDelay);
		VIM_HIF_SetReg8(V5_REG_LBUF_CAP_DLY_H,(UINT8)((wLineBuvDelay>>8)|BIT7));
		}
		else
			VIM_HIF_SetReg8(V5_REG_LBUF_CAP_DLY_H,0);
	}
	else
	{
		VIM_HIF_SetReg8(V5_REG_LBUF_CAP_DLY_L,0xff);
		VIM_HIF_SetReg8(V5_REG_LBUF_CAP_DLY_H,0x7f);
	}
	return VIM_SUCCEED;
}
/********************************************************************************
...
{
	Size->cx=VIM_HIF_GetReg8(V5_REG_JPEG_WIDTH_L);
	Size->cx|=(VIM_HIF_GetReg8(V5_REG_JPEG_WIDTH_H)<< 8);
	Size->cy=VIM_HIF_GetReg8(V5_REG_JPEG_HEIGHT_L);
	Size->cy|=(VIM_HIF_GetReg8(V5_REG_JPEG_HEIGHT_H)<< 8);
}
/********************************************************************************
  Description:
	get line buf width 
...
{
UINT16 dWidth;
	dWidth=VIM_HIF_GetReg8(V5_REG_LBUF_WIDTH_L);
	dWidth|=(VIM_HIF_GetReg8(V5_REG_LBUF_WIDTH_H)<< 8);
	return dWidth;
}
/********************************************************************************
  Description:
...
void VIM_JPEG_SetJpegInfo(PVIM_JPEG_Index pinfo)
{
	...
	//Use custom qt, resetstate before custom qt enable.
	VIM_JPEG_ResetState();//angela 2006-1-5
...
static 	const UINT8 gJpegIntPriority[3] = 
{ 
...
UINT8 i;
	if(bySecondLevelInt>=VIM_JPEG_JPEG_INT_ALL)
	{
		for(i=0;i<VIM_JPEG_JPEG_INT_ALL;i++)
			gVc0528_Isr.jpegisr[i] = Isr;
		return;
	}
	gVc0528_Isr.jpegisr[bySecondLevelInt] = Isr;
}
/********************************************************************************
  Description:
...
{
	UINT8 bInten;
	if(bySecondLevelInt<VIM_INT_JPEG_LINBUF_READ)
	{
		bInten=VIM_HIF_GetIntEnableSec(INT_JPEG);
		if(Eanble==ENABLE)
			bInten |= (0x1<<bySecondLevelInt);
		else
			bInten &= (~(0x1<<bySecondLevelInt));	
		VIM_HIF_SetIntEnableSec(INT_JPEG,bInten);
		if(bInten)
			VIM_HIF_SetIntModuleEn(INT_JPEG,ENABLE);
		else
			VIM_HIF_SetIntModuleEn(INT_JPEG,DISABLE);
	}
	else if(bySecondLevelInt<VIM_JPEG_JPEG_INT_ALL)
	{
		bInten=VIM_HIF_GetIntEnableSec(INT_LBUF);
		if(Eanble==ENABLE)
			bInten |= (0x1<<(bySecondLevelInt-3));
		else
			bInten &= (~(0x1<<(bySecondLevelInt-3)));	
		VIM_HIF_SetIntEnableSec(INT_LBUF,bInten);
		if(bInten)
			VIM_HIF_SetIntModuleEn(INT_LBUF,ENABLE);
		else
			VIM_HIF_SetIntModuleEn(INT_LBUF,DISABLE);
	}
	else
	{
		VIM_HIF_SetIntEnableSec(INT_JPEG,0xff);
		VIM_HIF_SetIntEnableSec(INT_LBUF,0xff);
		VIM_HIF_SetIntModuleEn(INT_JPEG,ENABLE);
		VIM_HIF_SetIntModuleEn(INT_LBUF,ENABLE);
	}
}
...
static const UINT8 gLbufIntPriority[VIM_JPEG_LBUF_INT_NUM] = { VIM_INT_JPEG_LINBUF_READ};
