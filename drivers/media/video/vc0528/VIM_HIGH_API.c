...
VIM_RESULT result;
	...
	if(result){
		VIM_USER_PrintString("V5-2: VIM_HAPI_InitVc05x error !!");
...
#if(VIM_HIGH_API_DEBUG)&&(VIM_528RDK_DEBUG)
	if(result)
		VIM_USER_PrintString("V5-2: have not find sensor");
	else
...
Parameters:
	Mode:
		 ...
		 VIM_HAPI_SPECIAL_MONOCHROME,	 //?ڰ?
		 VIM_HAPI_SPECIAL_SEPIA,		//????
		 VIM_HAPI_SPECIAL_NEGATIVE,	//??Ƭ
		 VIM_HAPI_SPECIAL_REDONLY,	 //??ɫͻ??
	Y = ( 77R + 150G + 29B)/256
