...
void VIM_HIF_SetReg16(UINT32 adr, UINT16 val)       
{
	...
	else
	{
		...
		*(volatile UINT16 *)(VIM_REG_INDEX) = (UINT16)adr-0x01;
		VIM_SET_RSHIGH_BY_GPIO();
		uTemp = *(volatile UINT16 *)(VIM_REG_VALUE);
		*(volatile UINT16 *)(VIM_REG_VALUE) = (uTemp&0xff) | (val<<8);	// 取低八位

		VIM_SET_RSLOW_BY_GPIO();
		*(volatile UINT16 *)(VIM_REG_INDEX) = (UINT16)adr+0x01;
		VIM_SET_RSHIGH_BY_GPIO();
		uTemp = *(volatile UINT16 *)(VIM_REG_VALUE);
		*(volatile UINT16 *)(VIM_REG_VALUE) = (uTemp&0xff00) | (val>>8);	// 取高八位
	}
}
/***************************************************************
Description:
...
V5_REG_BIU_SEL_8_16					(V5_REG_BIU_BASE+0x08C)
	...
	De fault value - 1’b0
********************************************************************************
Description:    //set the 8 /16 bit width  设置位宽并初始化
...
UINT8 VIM_HIF_GetIntFlagSec(VIM_HIF_INT_TYPE byOffset)        //二级  中断标志
{
	UINT8 x;
